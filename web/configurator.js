...
const allFiles = (await Array.fromAsync(Deno.readDir(CONFIG_DIR)))
...
await all(allFiles, async ([file, fileSource]) => {
  try {
    ...
    if (err instanceof Deno.DenoError && err.kind === Deno.ErrorKind.NotFound) {
      await saveConfig(file, parseConfig(await Deno.readTextFile(fileSource)))
...
const getConfigs = async files => {
  const [conf, defaultConf] = (await all(files, Deno.readTextFile)).map(parseConfig)
...
const getConfig = async (i, key) => {
  ...
  const config = parseConfig(await Deno.readTextFile(file))
